...
import { ROLES, ROLE_PAIRS, ROLE_TYPES } from '../roles';

/**
 * Migra i ruoli hardcoded da roles.js al database
 * Crea la storia "Chi è il Milionario?" con tutti i ruoli e obiettivi
 *
 * ATTENZIONE: Eseguire una sola volta!
 */
export async function migrateRolesToDatabase() {
  console.log('Starting migration of roles to database...');

  // 1. Crea la storia principale
...
Ogni giocatore ha un ruolo segreto con obiettivi da completare. Alcuni ruoli funzionano in coppia: scoprite il vostro partner e collaborate!`,
  ...
  // Mappa role_key -> role_id
  const roleIdMap = {};
  for (const role of roles) {
    roleIdMap[role.role_key] = role.id;
  }

  // 3. Crea gli obiettivi
...
/**
 * Crea una storia di esempio "La Spia Russa"
 * (per testare il sistema multi-storia)
 */
export async function createSpyStory() {
  console.log('Creating spy story...');
...
Fiducia, tradimento e sotterfugi: chi vincerà?`,
  ...
  // 2. Crea ruoli specifici per questa storia
  const spyRoles = [
    {
      story_id: story.id,
      role_key: 'spy',
...
export async function checkMigrationStatus() {
  ...
  return { migrated: !!data, story: data };
}
